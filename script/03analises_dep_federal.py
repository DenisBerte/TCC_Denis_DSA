# -*- coding: utf-8 -*-
"""
Created on Sun Aug 31 13:45:28 2025
...
dfdep_fed = dfdep[(dfdep["DS_CARGO"] == "DEPUTADO FEDERAL")][variaveis_rf]


# Salva o dataframe em um arquivo separado
dfdep_fed.to_excel("dfdep_fed.xlsx", index=False)


#%% ########################
# RANDOM FOREST - manipulações base federal

#%% abrir a base

dfdep_fed = pd.read_excel("dfdep_fed.xlsx")
...
conta_religiosos = dfdep_fed[dfdep_fed["RELIGIOSOS"] == 1].shape[0]

# Contar quantos têm SEGURANCA = 1
conta_seguranca = dfdep_fed[dfdep_fed["SEGURANCA"] == 1].shape[0]
...
religiosos_eleitos_ano = dfdep_fed[
    (dfdep_fed["RELIGIOSOS"] == 1) &
    (dfdep_fed["DS_SIT_TOT_TURNO"] == "ELEITO")
].groupby("ANO_ELEICAO").size()

# SEGURANCA eleitos por ano
seguranca_eleitos_ano = dfdep_fed[
    (dfdep_fed["SEGURANCA"] == 1) &
    (dfdep_fed["DS_SIT_TOT_TURNO"] == "ELEITO")
].groupby("ANO_ELEICAO").size()
...
df_eleitos_por_ano.to_excel("n_eleitos_federais.xlsx", index=True)


#%% Manipulações

# Garantir que 'DS_SIT_TOT_TURNO' esteja binária: 1 = ELEITO, 0 = NÃO ELEITO
dfdep_fed['DS_SIT_TOT_TURNO'] = (
    dfdep_fed['DS_SIT_TOT_TURNO']
...
dfdep_fed = pd.get_dummies(dfdep_fed, columns=['DS_GRAU_INSTRUCAO'], drop_first=True)

#%% FAIXA_ETARIA: 1='18–29', 2='30–44', 3='45–59', 4='60+'
dfdep_fed['FAIXA_ETARIA'] = (
    dfdep_fed['FAIXA_ETARIA']
...
dfdep_fed = pd.get_dummies(dfdep_fed, columns=['FAIXA_ETARIA'], drop_first=True)


#%% Variáveis categóricas para dummies

# Estado civil (referência: SOLTEIRO(A))
dfdep_fed = pd.get_dummies(dfdep_fed, columns=['DS_ESTADO_CIVIL'], drop_first=False)
dfdep_fed = dfdep_fed.drop(columns=['DS_ESTADO_CIVIL_SOLTEIRO(A)'])

# Espectro político (referência: Centro)
dfdep_fed = pd.get_dummies(dfdep_fed, columns=['ESPECTRO_POL'], drop_first=True)


# Grande região - (referência: Sudeste)
dfdep_fed = pd.get_dummies(dfdep_fed, columns=['REGIAO'], drop_first=False)
dfdep_fed = dfdep_fed.drop(columns=['REGIAO_SUDESTE'])


#%% # Converter todas as colunas booleanas para inteiras (int)

for col in dfdep_fed.columns:
    if dfdep_fed[col].dtype == bool:
        dfdep_fed[col] = dfdep_fed[col].astype(int)


#%%
print(dfdep_fed.dtypes)
print(dfdep_fed.head())
...
df_fed = dfdep_fed.copy()

# Salvar para analises
df_fed.to_excel("df_fed.xlsx", index=False)
...
# Processa somente colunas que existem no df_fed
colunas_categoricas = [c for c in mapas_categorias.keys() if c in df_fed.columns]

resumos = []
for col in colunas_categoricas:
    counts = df_fed[col].value_counts(dropna=False).sort_index()
...
df_resumo_final.to_excel('resumo_categorias_federais.xlsx', index=False)


#%% Separando variáveis X e y
X = df_fed.drop(columns=['DS_SIT_TOT_TURNO', 'ANO_ELEICAO'])
y = df_fed['DS_SIT_TOT_TURNO']

print(y.value_counts())

#%% Separando amostras de treino e teste
X_train = X[df_fed['ANO_ELEICAO'].isin([2014, 2018])]
y_train = y[df_fed['ANO_ELEICAO'].isin([2014, 2018])]

X_test = X[df_fed['ANO_ELEICAO'] == 2022]
y_test = y[df_fed['ANO_ELEICAO'] == 2022]
...
plt.title('Random Forest fed.: Treino')
...
plt.title('Random Forest fed.: Teste')
plt.xlabel('Classificado (Modelo)')
plt.ylabel('Observado (Real)')
plt.show()
...
#%% ################# Grid Search ########################################
# best params pós grid:{'max_depth': 15, 'max_features': 'sqrt', 
...
plt.title('Random Forest fed.: Treino (Após Grid Search)')
plt.xlabel('Classificado (Modelo)')
plt.ylabel('Observado (Real)')
plt.show()
...
plt.title('Random Forest fed.: Teste (Após Grid Search)')
plt.xlabel('Classificado (Modelo)')
plt.ylabel('Observado (Real)')
plt.show()
...
#%% Salvar tabela
rf_features.to_excel("RF_importancia_federal.xlsx", index=False)
...
plt.title('AUC-ROC Random Forest fed.: %g' % round(roc_auc_rf, 3), fontsize=22)
...
# Acrescenta OR e IC 95% se disponível
...
summary_df.to_excel("resultado_glm_fed.xlsx", index=True)
...
# salva
tabela.to_excel("tabela_sintetica_glm_fed.xlsx", index=False)
print("\nTabela sintética salva em: tabela_sintetica_glm_fed.xlsx")
...
#%% Ler base
df = pd.read_excel("tabela_sintetica_glm_fed.xlsx")
